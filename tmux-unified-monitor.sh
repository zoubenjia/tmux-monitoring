...
STATE_DIR="$HOME/.tmux-monitor/states"
NOTIFICATION_DIR="$HOME/.tmux-monitor/notifications"
CHECK_INTERVAL=3
NOTIFICATION_COOLDOWN=120  # 同一窗口的通知冷却时间（秒）

# 创建目录
[[ ! -d "$STATE_DIR" ]] && mkdir -p "$STATE_DIR"
[[ ! -d "$NOTIFICATION_DIR" ]] && mkdir -p "$NOTIFICATION_DIR"
